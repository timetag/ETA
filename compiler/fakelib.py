...
uettp_condition_max=8
uettp_symbols_max=10
uettp_tables_max=5
#otherinfo
uettp_condition_input_list=None
uettp_condition_output_list=None
#tables
uettp_tables=None
uettp_tables_num=None
uettp_tables_dim=None
#symbols
uettp_symbols=None
uettp_symbols_num=None
# init transition matrix
uettp_transition_matrix=None
# record
uettp_record_enable=None
#linear
uettp_linear_enable=None
uettp_linear_op=None
#signal
uettp_signal_enable=None
uettp_signal_op=None
#hist
uettp_hist_enable=None
uettp_hist_op=None
def uettp_clear_global():
    global uettp_tables
...
def uettp_clear():
    ...
    uettp_condition_input_list={}
    uettp_condition_output_list={}
  
    #symbols
    uettp_symbols={"uettp_initial":0}
    uettp_symbols_num=2
    # init transition matrix
    uettp_transition_matrix=tensor.make_tensor([uettp_condition_max,uettp_symbols_max])
    for each_condition in range(uettp_condition_max):
        for each_symbol in range(0,uettp_symbols_max,2):
            uettp_transition_matrix[each_condition][each_symbol]=get_complementry_symbol(each_symbol)
    for each_condition in range(uettp_condition_max):
        for each_symbol in range(1,uettp_symbols_max,2):
            uettp_transition_matrix[each_condition][each_symbol]=each_symbol
...
def table_resize(tableid,size=[1]):
    global uettp_tables_dim
    uettp_tables_dim[tableid]=tensor.max_dim(size,uettp_tables_dim[tableid])
...
def get_label(symbol):
    ...
    for each in uettp_symbols:
        if uettp_symbols[each]==symbol:
            ret.append(each)
    return ret
def get_complementry_symbol(symbol):
    if symbol%2==0:
        return symbol+1
    else:
        raise ValueError("WTF! This is already a complementry symbol. ")
"""
========ETA rev0=======
...
def symbol(name1=None,name2=None):
    ...
    if name1 is None and name2 is None:
        if uettp_symbols_num>=uettp_symbols_max:
                raise ValueError("A maximum number of created symbols is exceeded.")
    ...
    if name1 in uettp_symbols and name2 in uettp_symbols:
        if uettp_symbols[name1]==uettp_symbols[name2]:
            ...
            raise ValueError("Joint operation is not allowed for defined symbols. Symbol '"+str(name1) +"' or '"+str(name2)+ "' might appeared more than once in the graph.")
    else:
        ...
        if name1 is not None:
            if uettp_symbols_num>=uettp_symbols_max:
                raise ValueError("A maximum number of created tables is exceeded.")
            uettp_symbols[name1]=uettp_symbols_num
            if name2 is not None:
                uettp_symbols[name2]=uettp_symbols[name1]
            uettp_symbols_num+=2
...
def trans(condition,source=None,target=None):
    ...
    if uettp_transition_matrix[condition][source]==get_complementry_symbol(source):
        ...
        uettp_transition_matrix[condition][get_complementry_symbol(source)]=target
    else:
        raise ValueError("More than one transition from "+str(get_label(source))+" , when seeing signal "+str(condition)+" , are found. Which way it should go? Go to "+ str(get_label(target))+" , or go to "+str(get_label(uettp_transition_matrix[condition][source]))+"?")
...
def linear(trigger,saversymbol,mulp_a,symbol_x,mulp_b,symbol_y,offset_c=0):
    ...
    if uettp_linear_enable[trigger]==1:
        raise ValueError("More than one linear operation defined on "+str(get_label(trigger)))
...
def signal(trigger,signal,after=0,symbol_x=None,opertaor=None,symbol_y=None):
    ...
    if (signal==uettp_condition_max-1):
        raise ValueError("The last channel number is reserved for EOF signal.")
    global uettp_condition_output_list
    uettp_condition_output_list[signal]=signal
    global uettp_signal_enable
    global uettp_signal_op
    if uettp_signal_enable[trigger]==1:
        raise ValueError("More than one singal emission defined on "+str(get_label(trigger)))
    ...
    if symbol_x is not None:
        if (symbol_y is None) or (not(opertaor in operator_lut)):
            raise ValueError("Wrong condition statement for conditional signal.")
...
def hist(trigger,table,symbol,range_min,range_max,bin_step):
    ...
    if uettp_hist_enable[trigger]==1:
        raise ValueError("More than one histogram operation defined on "+str(get_label(trigger)))
...
def compile(uettp_json,log_cb=None):
    ...
    failed=None
    try:
        inslist=parser.compile_graph(uettp_json)+ parser.compile_usercode(uettp_json)
        inslist="\n".join(inslist)
        log_cb("CODE DUMP: \n"+inslist)
        exec(inslist)
        ...
        failed=str(e)
    if failed:
        ret={"uettp_condition_input_list":"N/A","uettp_condition_output_list":"N/A","failed":failed}
    else:

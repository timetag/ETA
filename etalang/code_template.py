...
def mainloop({tables} filename1, ReaderPTR1):
    ...
    for vslot in range(0,{num_vslot}):
        ...
        eta_ret += VFILE_init(nb.int64(vslot),nb.int64(size),VFILE,1)
...
def sp_core(caller_parms,mainloop):
    ...
    ReaderPTR1=np.zeros((15), dtype=np.int64);
    ReaderPTR1[0:7]=np.array(caller_parms,  dtype=np.int64)[0:7]
    
    print(mainloop( {tables}  bytearray(filename, "ascii"), ReaderPTR1))
  
    #fseekpoint, fendpoint, TTRes_pspr, SYNCRate_pspr, DTRes_pspr,RecordType,BytesofRecords
    return {table_list}

	...
	long long MKS_inline FileReader_pop_event(ttf_reader *READER_list, unsigned char RFILEid, unsigned char *out_Channel)
	{
		...
		while (true)
		{
			...
			case FORMAT_QT_COMPRESSED:
			{
				...
				AbsTime_ps = (*COMPTTTRRecordPtr).bits.time;
			...
			case FORMAT_BH_spc_4bytes:
			{
				...
				AbsTime_ps = ((*bh4bytesRecPtr).bits.macrotime + READER->overflowcorrection) * READER->SYNCRate_pspr + ((*bh4bytesRecPtr).bits.nanotime) * READER->DTRes_pspr;
			...
			case FORMAT_ET_A033:
			{
				...
				ETA033Struct *ETA033RecordPtr;
				ETA033RecordPtr = (ETA033Struct *)(READER->buffer + READER->next_RecID_in_batch * READER->BytesofRecords);
				if ((*ETA033RecordPtr).data1 > 0)
				{
					...
					{
						Channel = READER->CHANNEL_OFFSET + 1; // input B is chn 1
